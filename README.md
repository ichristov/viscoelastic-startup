...
Each notebook runs top to bottom on a fresh kernel and regenerates every number and figure it shows, except the values read off Balan's published figures, which are listed in the cell that uses them; there are no data files. On a laptop, each takes up to a couple of minutes. The notebooks are committed executed, so they can also be read on GitHub without running them, with a static plot in place of each interactive one. The regenerated paper figures are also in [`figures/`](figures/).
